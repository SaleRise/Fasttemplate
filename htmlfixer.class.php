...
Class HtmlFixer
{
    ...
    public function getFixedHtml($dirtyhtml)
    {
        ...
        while ($c < 10) {
            ...
            */
            if ($c > 0) {
                $this->dirtyhtml = $this->fixedxhtml;
            }
            $errorsFound = $this->charByCharJob();
            if (!$errorsFound) {
                $c = 10;
            }    // if no corrections made, stops iteration
        ...
        for ($i = 0; $i < count($o[1]); $i++) {
            ...
            if ($t != "") {
                ...
                if (($t[strlen($t) - 1] != "\"") && ($t[strlen($t) - 1] != "'")) {
                    $out = str_replace($t . $lc, $t . $q . $lc, $out);
                }
            }
        }
        return $out;
    ...
    private function fixTag($t)
    {
        ...
        for ($i = 0; $i < count($ar); $i++) {
            ...
            //if (stristr($ar[$i],"class=") || stristr($ar[$i],"style=")){
            $nt .= $ar[$i] . " ";
            /*}else{
    ...
    private function mergeStyleAttributes($s)
    {
        ...
        while (stristr($s, "style=\"")) {
            ...
            if ($c == 0) {
                $s = str_replace("style=\"" . $temp . "\"", "##PUTITHERE##", $s);
            }
        ...
        if ($c > 0) {
            $s = str_replace("##PUTITHERE##", "style=\"" . $x . "\"", $s);
        }
        return $s;
    ...
    private function checkTree()
    {
        ...
        for ($i = 1; $i < count($this->matrix); $i++) {
            ...
            if (in_array($this->matrix[$i]["tagType"], array("b", "strong"))) { //b cannot stay inside b o strong.
                $parentType = $this->matrix[$this->matrix[$i]["parentTag"]]["tagType"];
                if (in_array($parentType, array("b", "strong"))) {
                    $flag = true;
                }
            }
            ...
            if (in_array($this->matrix[$i]["tagType"], array("i", "em"))) { //i cannot stay inside i or em
                $parentType = $this->matrix[$this->matrix[$i]["parentTag"]]["tagType"];
                if (in_array($parentType, array("i", "em"))) {
                    $flag = true;
                }
            }
            ...
            if ($flag) {
                ...
                if ($this->debug) {
                    echo "<div style='color:#ff0000'>Found a <b>" . $this->matrix[$i]["tagType"]
                        . "</b> tag inside a <b>" . htmlspecialchars($parentType) . "</b> tag at node $i: MOVED</div>";
                }

                $swap = $this->matrix[$this->matrix[$i]["parentTag"]]["parentTag"];
                if ($this->debug) {
                    echo "<div style='color:#ff0000'>Every node that has parent " . $this->matrix[$i]["parentTag"]
                        . " will have parent " . $swap . "</div>";
                }
                $this->matrix[$this->matrix[$i]["parentTag"]]["tag"] = "<!-- T A G \""
                    . $this->matrix[$this->matrix[$i]["parentTag"]]["tagType"] . "\" R E M O V E D -->";
            ...
            if ($this->matrix[$i]["parentTag"] == $parentTag) {
                ...
                if ($this->matrix[$i]["tag"] != "") {
                    ...
                    $out .= "" . htmlspecialchars($this->matrix[$i]["tag"])
                        . "<span style='background-color:red; color:white'>{$i} <em>" . $this->matrix[$i]["tagType"]
                        . "</em></span>";
                ...
                if ($this->matrix[$i]["tag"] != "") {
                    ...
                    if ($this->matrix[$i]["tagType"] != "") {
                        if (($this->matrix[$i]["tagType"] != "br") && ($this->matrix[$i]["tagType"] != "img")
                            && ($this->matrix[$i]["tagType"] != "hr")
                            && ($this->matrix[$i]["tagType"] != "input")
                        ) {
                            $out .= "<div style='color:red'>" . htmlspecialchars("</" . $this->matrix[$i]["tagType"]
                ...
                if ($s[$i] == ">") {
                    ...
                    */
                    ...
                    if (($tags[$tagType] == 0) && (stristr($tag, '/' . $tagType . '>'))) {
                        ...
                        if ($this->debug) {
                            echo "<div style='color:#ff0000'>Found a closing tag <b>" . htmlspecialchars($tag)
